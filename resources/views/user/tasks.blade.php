...
show" role="alert">
        ...
        <div class="row g-3 align-items-center mb-4">
            <div class="col-md-4 d-flex align-items-center">
                ...
                <div>
                    <h1 class="h3 mb-0 text-dark">Daftar
                        Tugas</h1>
                    <p class="text-muted small mb-0">
                        <i class="fas fa-clipboard-list me
1"></i>
            ...
            <div class="col-md-5">
                <form action="{{ route('tasks.index') }}" method="GET" class="position-relative">
                    <div class="input-group shadow-gray-50 
inline">
                        <input type="text" class="form-control 
border-end-0 pe-0"
                            name="search"placeholder="Cari tugas..." value="{{ request('search') }}" id="searchInput">
                        <button type="submit" class="btn btn
secondary border-0 " id="clearSearchBtn"> <i
                                class="fas fa
search"></i> </button>
                    </div>
                </form>
            ...
            <div class="col-md-3 text-md-end d-flex gap-2">
                <button class="btn btn-primary shadow-sm flex
grow-1" data-bs-toggle="modal" data-bs-target="#addTaskModal">
        ...
        <div class="card border-0 shadow-sm">
            ...
            center justify-content-center {{ !request('status') ? 'active' : '' }}"
                            href="{{ route('tasks.index') }}" style="min-width: 100px;">
                            <i class="fas fa-list me-2"></i>
                            Semua
                            <span class="badge rounded-pill bg
            secondary ms-2">{{ $tasks->count() }}</span>
                        </a>
                    </li>
                    <li class="nav-item flex-grow-1 text-center">
                        <a class="nav-link d-flex align-items
            ...
            primary ms-2">{{ $tasks->where('status', 'tertunda')->count() }}</span>
                        </a>
                    </li>
                    <li class="nav-item flex-grow-1 text-center">
                        <a class="nav-link d-flex align-items
            ...
            2"></i>
                            ...
                            <span
                                class="badge rounded-pill bg
            success ms-2">{{ $tasks->where('status', 'selesai')->count() }}</span>
                        </a>
                    </li>
                </ul>
            </div>
        ...
        <div class="card-body p-0">
            @if ($tasks->isEmpty())
                <div class="text-center py-5 bg-white">
                    <div class="py-3">
                        <i class="fas fa-tasks fa-4x text-muted 
            mb-3"></i>
                        <h4 class="text-gray-900">Belum Ada
                            Tugas</h4>
                        <p class="text-muted mb-0">Mulai dengan
            ...
            light btn-sm border shadow-none"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Tugas
                        Baru</h5>
            ...
            label">Deskripsi<span class="text-danger">*</span></label>
             ...
             ('Deskripsi tugas harus diisi')"
                        ...
                        <div class="mb-3">
                            <label class="form-label">Tenggat
                                Waktu<span class="text-danger">*</span></label>
                            <input type="date" class="form
control" name="deadline" required
                                oninvalid="this.setCustomValidity
...
secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">Judul
                                    Tugas</label>
                                <input type="text" class="form
control" name="judul" value="{{ $task->judul }}"
                                    required>
                            </div>
                            <div class="mb-3">
                                <label class="form
...
primary">Simpan Perubahan</button>
    ...
    warning display-3 mb-3"></i>
                        <h4>Hapus Tugas?</h4>
                        <p class="text-muted">Apakah Anda yakin
                            ingin menghapus tugas:<br />"{{ $task->judul }}"?</p>
                        <p class="small text-danger">Tindakan ini
                            tidak dapat dibatalkan</p>
                    </div>
                    <div class="modal-footer border-0 justify
    content-center pb-4">
                        <button type="button" class="btn btn
    secondary px-4" data-bs-dismiss="modal">Batal</button>
                        <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn
    danger px-4">Hapus</button>
                        </form>
                    </div>
                </div>
            </div>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Judul
                                Tugas</label>
                            <input type="text" class="form
    control" name="judul" id="edit_judul" required>
                        </div>
                        <div class="mb-3">
                            <label class="form
    label">Deskripsi</label>
                        ...
                        <div class="mb-3">
                            <label class="form-label">Tenggat
                                Waktu</label>
                            <input type="date" class="form
    control" name="deadline" id="edit_deadline">
                        </div>
                        <div class="mb-3">
                            <label class="form
    label">Status</label>
    ...
    @push('scripts')
        <script>
            (() => {
                    ...
                    const forms = document.querySelectorAll('.needs
                    ...
                    })() document.getElementById('searchTask').addEventListene r('keyup', function(e) {
                        ...
                        const tasks = document.querySelectorAll('.list
                            ...
                            link.addEventListener('click', function(e) {
                                    ...
                                    document.querySelectorAll('.nav
                                        ...
                                        active ')); 
                                        this.classList.add('active'); tasks.forEach(task => {
                                            const isCompleted =
                                                task.querySelector('.form-check-input').checked;
                                            if (filter === 'all') {
                                                task.style.display = '';
                        ...
                        function editTask(id) {
                            fetch(`/tasks/edit/${id}`)
                                ...
                                .then(task => {
                                    document.getElementById('edit_judul').val
                                    ue = task.judul;
                                    document.getElementById('edit_deskripsi')
                                        .value = task.deskripsi;
                                    document.getElementById('edit_deadline').
                                    value = task.deadline.split(' ')[0];
                                    document.getElementById('edit_status').va
                                    lue = task.status;
                                    const editForm =
                                        document.getElementById('editTaskForm');
                                    editForm.action = `/tasks/${id}`;
                                    new
                                    bootstrap.Modal(document.getElementById('editTaskModal')).show();
                                })
                                .catch(error => {
                                        alert('Terjadi kesalahan saat mengambil 
                                            data tugas '); 
